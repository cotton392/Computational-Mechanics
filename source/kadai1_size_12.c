...
int is_convergence(float temp_field[13][13][2]) {
  // 収束判定関数
  for(int y=0;y<13;y++) {
    for(int x=0;x<13;x++) {
...
void temp_field_copy(float temp_field[13][13][2]) {
  // 温度分布のコピー関数
  for(int y=0;y<13;y++) {
    for(int x=0;x<13;x++) {
...
void print_temp_field(float temp_field[13][13][2], float strict_temp_field[13][13], bool cond) {
  // 温度分布の出力関数
  for(int y=0;y<13;y++) {
      for(int x=0;x<13;x++) {
...
int main() {
  float temp_field[13][13][2]; // [y][x][0]は新しい計算結果を格納, [y][x][1]は古い計算結果を格納
  float strict_temp_field[13][13];
  ...
  for(int y=0;y<13;y++){
    for(int x=0;x<13;x++) {
      if(x == 0 || y == 12 || x == 12) {
        ...
        temp_field[y][x][0] = 50 * sin((M_PI / 12) * x); // 両端で0になるように分割数で割る
        temp_field[y][x][1] = 50 * sin((M_PI / 12) * x);
        strict_temp_field[y][x] = 50 * sin((M_PI / 12) * x);
      } else {
        temp_field[y][x][0] = -1; // 未計算領域を-1とする
        temp_field[y][x][1] = -1;
        strict_temp_field[y][x] = (50 / sinh(M_PI)) * sin((M_PI / 12) * (12-x)) * sinh((M_PI / 12) * (12-y));
      }
    }
  }
